...
\end{theorem}
This \emph{O$(\log k)$-competitive} bound predicts that the advantage of $k$-means++
grows with $k$ — verified empirically in Section~\ref{sec:exp3}.
...
$k$-means++ dominates on every axis. It reduces mean inertia by \textbf{60.4\%}
...
The near-optimal fraction rises from 42\% to \textbf{88\%}: random init fails to find a
near-optimal solution in more than half of runs.
...
Lloyd's updates cannot undo this — once centers are placed, every assignment step is
optimal \emph{given} those centers, locking in the sub-optimal partition. $k$-means++
...
\begin{figure}[H]
  ...
  \caption{Left: mean inertia vs $k$. Right: random/$k$-means++ inertia ratio vs $k$.
           ...
           tracking $\ln k$, directionally consistent with the O$(\log k)$ theory.}
  \label{fig:exp3}
\end{figure}

\begin{table}[H]
\centering\small
\caption{Inertia ratio vs $k$: $\bar{J}_\text{rand}/\bar{J}_{++}$ grows with $\ln k$.}
\label{tab:exp3}
\begin{tabular}{ccccc}
\toprule
...
\textbf{1.815} at $k=15$, tracking $\ln k$ closely (Table~\ref{tab:exp3}).
For small $k$ (2--5) the advantage is negligible: with few clusters, random init rarely
seeds two centers in the same cluster. At $k=7$ the ratio jumps to 1.22 as seeding
errors become frequent. This is directionally consistent with the O$(\log k)$ competitive bound:
more clusters give random init more opportunities to fail, widening the gap.
...
\begin{figure}[H]
  ...
  \caption{Digits 64-D: nearly identical, very tight inertia distributions (CV
           $\approx1.3\%$ for both). The objective landscape is effectively unimodal.}
  \label{fig:exp4}
\end{figure}

The result \textbf{inverts}. $k$-means++ is marginally \emph{worse}: inertia $+1.3\%$,
ARI 0.464 vs \textbf{0.512}, NMI 0.618 vs \textbf{0.656}. Three mechanisms contribute:

\begin{enumerate}[nosep,leftmargin=*]
  \item \textbf{Distance concentration.} In 64 dimensions, all pairwise distances become
        similar ($\max/\min\to1$), flattening the $D^2$ distribution so $k$-means++
        sampling is nearly uniform — its seeding signal vanishes.
  \item \textbf{Landscape unimodality.} CV $\approx1.3\%$ for \emph{both} methods —
        50$\times$ lower than Experiment 1. The landscape has effectively one basin;
        initialization choice is irrelevant for final inertia.
  \item \textbf{Greedy spreading on outliers.} $k$-means++ actively maximizes spread,
        which can anchor seeds on atypical digit images. Such outlier seeds attract few
...
The experiments tell a coherent, theory-consistent story.

\textbf{Finding 1 — Strong structure: $k$-means++ wins decisively.}
A 60.4\% inertia reduction, ARI from 0.833 to 0.966, NMI from 0.919 to 0.984, half the
iterations, and 88\% near-optimal rate vs 42\% for random init. The scatter plots confirm
the mechanism: $D^2$ seeding prevents the pathological two-seeds-in-one-cluster
initialization that Lloyd cannot recover from.
...
On 64-D Digits, $k$-means++ is marginally worse on inertia, ARI, and NMI. Distance
concentration eliminates the $D^2$ signal; the landscape becomes near-unimodal; and
greedy spreading can misplace seeds on outlier images. This negative result is only
visible through external metrics.

\textbf{Finding 4 — Growing advantage with $k$, matching theory.}
The inertia ratio grows from 1.01 at $k=2$ to 1.82 at $k=15$, tracking $\ln k$ and
directionally consistent with the O$(\log k)$ competitiveness guarantee.
...
low-dimensional structure and large $k$, matching the O$(\log k)$ theory.
...
(ii)~PCA pre-processing on Digits might reduce distance concentration and restore the advantage.
